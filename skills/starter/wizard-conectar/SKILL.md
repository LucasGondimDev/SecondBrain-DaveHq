...
**Validação Tavily (smoke test visível — Princípio 14):**
...
# 2. Validar com call real (mostrar comando + output literal)
curl -s -X POST "https://api.tavily.com/search" \
  -H "Content-Type: application/json" \
  -d "{\"api_key\":\"$TAVILY_API_KEY\",\"query\":\"test\",\"max_results\":1}" \
  -w "\nHTTP %{http_code}\n"

# 3. 200 OK = válida · 401 = inválida · 429 = rate limit
```

> ⚠️ **Atenção known key OpenClaw:** se `TAVILY_API_KEY` ainda NÃO estiver na allow-list de known keys do Hostinger Managed (verificar com `openclaw secrets list` após o reload), a chave NÃO vai ficar acessível pro agente nativo. Workaround: configurar via `export TAVILY_API_KEY=...` no shell + agente usa SDK Python (`pip install tavily-python`) em vez do daemon nativo. Esse caminho é mais flexível e independe da Hostinger atualizar known keys.
